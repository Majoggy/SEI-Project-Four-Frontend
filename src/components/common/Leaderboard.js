...
import { percentage, minusFormatting } from '../../lib/helpers'
...
function Leaderboard({ games }) {
  ...
  React.useEffect(() => {
    const getData = async () => {
      try {
        ...
        setPlayers(res.data)
        console.log(massageData(games, players))
      } catch (err) {
        console.log(err)
      }
    }
  ...
  function massageData(games, players) {
    const newData = []
    ...
    players.map(player => {
      ...
      playerObj = { ...playerObj, 'total': total, gamesPlayed: gameCount, 
